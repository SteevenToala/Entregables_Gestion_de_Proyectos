...
La confiabilidad y rigor del sistema \textbf{SIGE} radica en su capacidad para garantizar el cumplimiento normativo institucional del Sistema de Gestión de la Calidad (SGC). En este documento se especifican formalmente las \textbf{Reglas de Negocio (RN)} y la arquitectura de compuertas secuenciales (\textbf{Validation Gates}) implementadas de forma determinista en cliente (React + Zod) y servidor (Node.js + Express v5 con middlewares modulares de validación y PostgreSQL 18) que impiden el avance, guardado o envío de documentos con inconsistencias aritméticas o documentales.

\section{Objetivo}

\subsection{Objetivo General}
Formalizar y parametrizar las reglas de negocio institucionales y compuertas de validación deterministas que rigen los flujos de estado, cálculo de ponderaciones, carga obligatoria de evidencias y firmas digitales en SIGE.

\subsection{Objetivos Específicos}
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, topsep=2pt]
    \item Especificar las 9 reglas de negocio deterministas del SGC con sus formulaciones matemáticas y validaciones en middlewares de Express (v5).
    \item Definir los 4 Validation Gates secuenciales: Gate 1 (Estructura), Gate 2 (Evidencias MinIO), Gate 3 (Ponderación 100\%) y Gate 4 (Firma Previa Válida).
    \item Modelar el diagrama de flujo determinista de compuertas lógicas.
\end{enumerate}

\section{Alcance}

\subsection{Límites de Aplicación (In-Scope)}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item Aplica a todos los documentos generados bajo los formatos P7-T1 (Plan de Trabajo) y P7-T2 (Informe de Cumplimiento) en la FISEI.
    \item Validación reactiva en frontend (React + Zod) y validación estricta transaccional en backend (Node.js + Express v5 y PostgreSQL 18).
\end{itemize}

\subsection{Exclusiones (Out-of-Scope)}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item No aplica a resoluciones disciplinarias o trámites ajenos a planes e informes académicos.
\end{itemize}

\section{Definiciones}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item \textbf{Validation Gate (VG):} Compuerta lógica evaluada por el backend que bloquea el cambio de estado si alguna precondición falla.
    \item \textbf{Regla de Negocio (RN):} Restricción imperativa que condiciona la estructura, cálculos o transiciones de datos del sistema.
    \item \textbf{Payload Tampering Protection:} Mecanismo de seguridad que rechaza valores enviados por el cliente para campos que deben ser generados exclusivamente por el servidor.
    \item \textbf{Inmutabilidad:} Condición de un registro que impide cualquier modificación posterior una vez legalizado.
\end{itemize}
...
\begin{tabularx}{\textwidth}{|>{\hsize=0.6\hsize\bfseries}X|>{\hsize=0.9\hsize}X|>{\hsize=1.5\hsize}X|}
    ...
    \hline
    Steeven Toala & Líder de Proyecto / Arquitecto & Diseño de compuertas lógicas y orquestación del flujo de validación. \\
    \hline
    Nixon Hurtado & Backend / DBA & Implementación de las reglas en middlewares de Express (v5) y restricciones en PostgreSQL 18. \\
    \hline
    Ing. Paulo Torres & Docente Tutor & Validación de correspondencia normativa con los estándares del SGC de la UTA. \\
    \hline
\end{tabularx}
\caption{Matriz de Responsabilidades del Entregable EDT 1.5.}
\end{table}

\section{Normativa Legal y Técnica}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item \textbf{Reglamento de Régimen Académico CES:} Parámetros de dedicación horaria y cumplimiento de actividades docentes.
    \item \textbf{Norma ISO 9001:2015:} Cláusula 8.5.1 (Control de la producción y de la provisión del servicio).
    \item \textbf{Manual del SGC - FISEI UTA:} Criterios de evaluación y ponderación de matrices de trabajo.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em, itemsep=1.5pt]
    ...
    \item \textbf{Validación Backend (Middleware Modular Express v5):}
\end{itemize}
\begin{lstlisting}[language=Java]
// backend/src/modules/reports/report.validation.js
export const validateReportEvidences = (req, res, next) => {
  const { actividades } = req.body;
  for (const act of actividades) {
    if (act.porcentajeAvance > 0 && (!act.evidencias || act.evidencias.length === 0)) {
      return res.status(400).json({
        status: 'error',
        message: 'Toda actividad con avance mayor a 0% requiere evidencia cargada en MinIO.'
      });
    }
  }
  next();
...
\begin{lstlisting}[language=Java]
// backend/src/modules/reports/report.validation.js
export const validateReportDeviations = (req, res, next) => {
  const { actividades } = req.body;
  for (const act of actividades) {
    if (act.porcentajeAvance < 100 && (!act.observaciones || act.observaciones.trim().length < 10)) {
      return res.status(400).json({
        status: 'error',
        message: 'Actividades con avance < 100% deben incluir justificacion de al menos 10 caracteres.'
      });
    }
  }
  next();
...
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}m{2.9cm}|>{\raggedright\arraybackslash}X|>{\centering\arraybackslash}m{4.6cm}|>{\centering\arraybackslash}m{2.3cm}|}
    ...
    \hline
    \textbf{Elaborado por:} & Steeven Toala\newline \textit{Líder de Proyecto / Arquitecto Software} & \parbox[c]{4.4cm}{\centering\vspace{4pt}\includegraphics[height=1.5cm,width=4.0cm,keepaspectratio]{images/firma_steveen.png}\vspace{4pt}} & 20/08/2026 \\
    \hline
...
\section{Anexos}
\subsection*{Anexo 1: Middlewares Modulares en Express (v5) para Validation Gates}
Cadena de middlewares en Node.js + Express (v5) que intercepta las peticiones HTTP y evalúa de forma determinista las compuertas de validación de esquemas y reglas de negocio antes de la persistencia en PostgreSQL 18.
