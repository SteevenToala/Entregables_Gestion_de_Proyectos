...
Planificar, delimitar y formalizar el alcance integral del sistema web SIGE, asegurando la correspondencia entre los objetivos estratégicos institucionales y los requerimientos técnicos mediante la Matriz de Trazabilidad (RTM).
...
\subsection{Límites del Sistema (In-Scope)}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    ...
    \item \textbf{Generación Oficial de Documentos:} Motor de exportación fidedigna a formatos oficiales PDF y Word \texttt{.docx}.
...
\subsection{Exclusiones Explícitas (Out-of-Scope)}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item \textbf{In-Scope:} Funcionalidades, módulos y artefactos contractuales que serán desarrollados y entregados.
    \item \textbf{Out-of-Scope:} Límites y exclusiones que no forman parte del esfuerzo de desarrollo del presente proyecto.
    \item \textbf{RTM (\textit{Requirements Traceability Matrix}):} Matriz que vincula cada objetivo con sus requerimientos funcionales y criterios de aceptación.
    \item \textbf{Scope Creep:} Incremento no controlado del alcance sin ajuste de tiempo o recursos.
\end{itemize}
...
\begin{tabularx}{\textwidth}{|>{\hsize=0.6\hsize\bfseries}X|>{\hsize=0.9\hsize}X|>{\hsize=1.5\hsize}X|}
    ...
    \hline
    Steeven Toala & Líder de Proyecto / Arquitecto & Delimitación del alcance técnico, control de restricciones y matriz RTM. \\
    \hline
    Heidy Villavicencio & Analista de Requerimientos & Formulación de objetivos SMART y mapeo de supuestos operativos. \\
    \hline
    Ing. Paulo Torres & Docente Tutor & Validación y aprobación de los límites del alcance institucional. \\
    \hline
\end{tabularx}
\caption{Matriz de Responsabilidades del Entregable EDT 1.4.}
\end{table}

\section{Normativa Legal y Técnica}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item \textbf{PMBOK Guide 7th Edition:} Estándar de Gestión del Alcance y Creación de la EDT.
    \item \textbf{ISO/IEC/IEEE 12207:2017:} Procesos del ciclo de vida del software -- Proceso de Definición del Alcance.
    \item \textbf{Estatuto Orgánico UTA:} Lineamientos de gestión documental universitaria.
\end{itemize}

\section{Método, Políticas y Contenidos}
Se aplicó la descomposición estructurada del alcance basada en las especificaciones del Sistema de Gestión de Calidad (SGC) institucional:
...
\subsection{Restricciones y Supuestos del Proyecto}
\begin{enumerate}[leftmargin=1.5em, itemsep=2.5pt]
    \item \textbf{Restricción Presupuestaria:} El presupuesto de inversión directa está limitado a \textbf{\$630.30 USD} para infraestructura cloud (VPS, PostgreSQL 18, MinIO, dominio/SSL, SMTP y herramientas de soporte). La mano de obra se valoriza en \textbf{\$0.00 USD} como aporte académico de los 4 estudiantes de ingeniería.
    ...
    \item \textbf{Restricción Tecnológica:} Stack tecnológico basado en Node.js + Express (v5), PostgreSQL 18, React + Vite con Tailwind CSS, Bcrypt.js, JWT y Helmet. Asistente ortográfico local en contenedor Docker (LanguageTool) y asistencia semántica mediante Google Gemini API sin costes de suscripción comercial.
    ...
    \item \textbf{Supuestos Operativos:} Los docentes cuentan con certificados de firma electrónica vigentes emitidos por entidades certificadoras acreditadas en el país (FirmaEC, Security Data, Banco Central, etc.); el servidor VPS cuenta con mínimo 4 vCPUs y 8 GB RAM.
\end{enumerate}

\subsection{Matriz de Trazabilidad de Requerimientos (RTM)}
...
\begin{tabularx}{\textwidth}{|>{\hsize=0.35\hsize\bfseries\centering\arraybackslash}X|>{\hsize=0.85\hsize}X|>{\hsize=0.9\hsize}X|>{\hsize=0.9\hsize}X|}
    ...
    \hline
    \textbf{OE-02} & RF-07, RF-08, RF-09, RF-16 & Prototipos Figma y Componentes React + Vite & Interfaz con navegación por pestañas funcional, sobria y accesible. \\
    \hline
    \textbf{OE-03} & RF-10, RF-11, RF-12, RF-13, RF-14 & Middlewares de Validación en Node.js + Express v5 & Bloqueo determinista de guardado si faltan evidencias o ponderaciones. \\
    \hline
    ...
    \hline
    \textbf{OE-06} & RF-22, RF-23, RF-24 & Módulo Exportación PDF/Word y Logs SHA-256 & Documentos exportados idénticos a los formatos oficiales del SGC. \\
    \hline
\end{tabularx}
...
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}m{2.9cm}|>{\raggedright\arraybackslash}X|>{\centering\arraybackslash}m{4.6cm}|>{\centering\arraybackslash}m{2.3cm}|}
    ...
    \hline
    \textbf{Elaborado por:} & Steeven Toala\newline \textit{Líder de Proyecto / Arquitecto Software} & \parbox[c]{4.4cm}{\centering\vspace{4pt}\includegraphics[height=1.5cm,width=4.0cm,keepaspectratio]{images/firma_steveen.png}\vspace{4pt}} & 19/08/2026 \\
    \hline
