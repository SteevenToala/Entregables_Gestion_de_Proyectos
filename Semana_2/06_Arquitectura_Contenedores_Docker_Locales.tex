...
Para garantizar reproducibilidad en los entornos de desarrollo, pruebas y producción sin incurrir en costos de licencias comerciales ni saturar los recursos de hardware, el sistema \textbf{SIGE} implementa una arquitectura eficiente y desacoplada mediante \textbf{Docker Compose}: servicios locales contenerizados (\textbf{PostgreSQL 18}, MinIO, LanguageTool, \textbf{Backend Node.js + Express v5}, \textbf{Frontend React + Vite}) e inferencia semántica delegada a la \textbf{API de Google Gemini (Gemini 2.5 Flash)} a través de HTTPS sin costo de GPU local.
...
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt, topsep=2pt]
    \item Configurar los 6 servicios del \texttt{docker-compose.yml} (PostgreSQL 18, MinIO, MinIO-Init, LanguageTool, Backend Node.js Express v5 y Frontend React + Vite).
...
\begin{tabularx}{\textwidth}{|>{\hsize=0.65\hsize\bfseries}X|>{\hsize=0.85\hsize}X|>{\hsize=1.5\hsize}X|}
    ...
    \hline
    Nixon Hurtado & Backend / DBA & Configuración del contenedor de PostgreSQL 18 y persistencia de volúmenes. \\
    \hline
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item \textbf{Servicios Locales Contenerizados:} Base de datos relacional (\textbf{PostgreSQL 18}), almacenamiento de objetos S3 (\textbf{MinIO}), corrector ortográfico/gramatical local de alta velocidad (\textbf{LanguageTool}), backend REST (\textbf{Node.js + Express v5}) y cliente web (\textbf{React + Vite + Nginx}).
    \item \textbf{Asistencia Semántica de Inteligencia Artificial:} La inferencia semántica para redacción institucional se delega a la \textbf{API de Google Gemini (Gemini 2.5 Flash)} a través de peticiones seguras HTTPS (REST API), optimizando la infraestructura local a cero sobrecosto de hardware.
\end{itemize}
...
services:
  # 1. Base de Datos Relacional PostgreSQL 18
  sige-postgres:
  ...
  # 5. Backend REST API (Node.js + Express v5)
  sige-backend:
    build:
      context: ./backend
    ...
    environment:
      ...
      - PORT=5000
      - DB_HOST=sige-postgres
      - DB_PORT=5432
      - DB_NAME=sige_db
      - DB_USER=sige_admin
      - DB_PASSWORD=SigeSecurePassword2026!
      - MINIO_ENDPOINT=sige-minio
      - MINIO_PORT=9000
      - MINIO_ACCESS_KEY=sige_minio_admin
      - MINIO_SECRET_KEY=SigeMinioSecretKey2026!
      - MINIO_BUCKET=sige-evidencias-bucket
      - LANGUAGETOOL_URL=http://sige-languagetool:8010
      - GEMINI_API_KEY=${GEMINI_API_KEY}
      - JWT_SECRET=SigeSuperSecretKeyForTokenSigning2026UTAFISEISoftwareEngineering!
      - JWT_EXPIRES_IN=30m
    ports:
      - "5000:5000"
    networks:
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    ...
    \item \textbf{Backend REST API:} \texttt{http://localhost:5000/api/health}
